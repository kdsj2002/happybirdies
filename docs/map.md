...
팀 쪽으로 끌면 그 자리 칩과 맞바뀐다(`swapChip`). 이긴 팀은 이름표
(`data-rspick`, 칩이 아닌 쪽)를 누르면 선택되고(`winSide`) 트로피(🏆)가
붙는다.
...
보여준다(손잡이엔 일부러 숫자를 안 넣는다 — 드래그 중엔 손가락에 가려
안 보이므로). `opts.onSave(result, roster)`로 콜백하며, 코트 종료·
리매치·기록 화면 수정이 전부 이 함수를 공유한다.
