...
const Auth = (() => {
  ...
  const ADMIN_ONLY = new Set(['courtAssign','settings','closeSess']);
  ...
  return {
    ...
    async restore(){
      ...
      if(r === 'member'){
        ...
        if(id && S.members.some(m=>m.id===id)){ role='member'; memberId=id; return true; }
    ...
    /* 권한 판정 —
       edit        보드 조작 전반(출석/시작/종료/대기열/자동정렬)
       courtAssign 코트에 사람을 직접 넣고 빼는 수동 배정  → 운영자만
       settings    설정 변경 (코트 수, 가중치, 성별 정책 등)  → 운영자만
       members     회원 명단 화면
       closeSess   세션 마감                                → 운영자만 */
    can(action){
      if(role === 'admin')  return true;
      if(role === 'member') return !ADMIN_ONLY.has(action);
      return action === 'view';
    },
    denyMsg(action){
      if(role === 'viewer') return '보기 전용으로 입장했습니다';
      if(action === 'courtAssign') return '코트 수동 배정은 운영자만 할 수 있습니다';
