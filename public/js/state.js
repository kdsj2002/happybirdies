...
/* ── 경기 기록과 결과 ──────────────────────────────────────────────
   ...
   왜 이렇게 나눠 두는가.
     ...
     · 진 팀 점수(sl)는 결과 입력 창(interact.js resultDialog)에서 사람이
       직접 고른 값을 그대로 받는다. 이긴 팀 점수(sw)는 그 값에 따라
       갈린다 — 진 팀이 21점 미만이면 21점제였는지 25점제였는지 점수만으론
       가릴 수 없어 사람이 21점 또는 25점 중 직접 고르고, 21점 이상이면
       이미 그 점수제를 넘겨 이어가는 판이므로 25점, 25점 이상이면 그마저
       넘긴 연장 듀스이므로 진 팀 점수+1점으로 화면이 자동으로 채운다
       (`interact.js` `autoWinScore`).
